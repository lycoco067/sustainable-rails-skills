...
### `/sus-test scenario` - 자동 테스트 + 오류 수정 + 스크린샷
...
```bash
/sus implement 리더보드 기능
```

**복잡한 기능을 Wave 단위로 병렬 처리:**
...
- **병렬 실행 모드** - Wave 단위로 작업을 분할하여 ~50% 시간 절약
- **자동 라우팅** - 키워드 분석 후 적절한 하위 에이전트로 위임
- **Quality Gate** - 작업 완료 후 원칙 위반 자동 감지 + 수정 제안
- **컨텍스트 관리** - 80% 도달 시 자동 /compact 실행
